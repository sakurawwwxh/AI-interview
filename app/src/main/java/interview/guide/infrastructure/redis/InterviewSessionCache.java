...
/**
 * 面试会话 Redis 缓存服务
 * 管理面试会话在 Redis 中的存储，缓存 key 包含 userId 维度实现物理隔离。
 */
@Slf4j
@Service
...
public class InterviewSessionCache {
    ...
    public void saveSession(Long userId, String sessionId, String resumeText, Long resumeId,
        ...
        log.debug("会话已缓存: userId={}, sessionId={}, resumeId={}, status={}", userId, sessionId, resumeId, status);
    }

    /**
     * 获取缓存的会话
     */
    public Optional<CachedSession> getSession(Long userId, String sessionId) {
        String key = buildSessionKey(userId, sessionId);
        CachedSession session = redisService.get(key);
        if (session != null) {
            log.debug("从缓存获取会话: userId={}, sessionId={}", userId, sessionId);
     ...
     * 更新会话状态
     */
    public void updateSessionStatus(Long userId, String sessionId, SessionStatus status) {
        getSession(userId, sessionId).ifPresent(session -> {
            ...
            if (!isUnfinishedStatus(status) && session.getResumeId() != null) {
                removeResumeSessionMapping(userId, session.getResumeId(), sessionId);
            }

            log.debug("更新会话状态: userId={}, sessionId={}, status={}", userId, sessionId, status);
        });
    }
     ...
     * 更新当前问题索引
     */
    public void updateCurrentIndex(Long userId, String sessionId, int currentIndex) {
        getSession(userId, sessionId).ifPresent(session -> {
            session.setCurrentIndex(currentIndex);
            String key = buildSessionKey(userId, sessionId);
            redisService.set(key, session, SESSION_TTL);
            log.debug("更新会话进度: userId={}, sessionId={}, currentIndex={}", userId, sessionId, currentIndex);
        });
    }
     ...
     * 更新问题列表（用于保存答案）
     */
    public void updateQuestions(Long userId, String sessionId, List<InterviewQuestionDTO> questions) {
        getSession(userId, sessionId).ifPresent(session -> {
            try {
                session.setQuestionsJson(objectMapper.writeValueAsString(questions));
                String key = buildSessionKey(userId, sessionId);
                redisService.set(key, session, SESSION_TTL);
                log.debug("更新会话问题: userId={}, sessionId={}", userId, sessionId);
            } catch (JacksonException e) {
    ...
    public void deleteSession(Long userId, String sessionId) {
        ...
        log.debug("删除会话缓存: userId={}, sessionId={}", userId, sessionId);
    }
    ...
    public Optional<String> findUnfinishedSessionId(Long userId, Long resumeId) {
        ...
        if (sessionId != null) {
            // 验证会话是否仍然存在且未完成
            Optional<CachedSession> sessionOpt = getSession(userId, sessionId);
     ...
     * 刷新会话过期时间
     */
    public void refreshSessionTTL(Long userId, String sessionId) {
        String key = buildSessionKey(userId, sessionId);

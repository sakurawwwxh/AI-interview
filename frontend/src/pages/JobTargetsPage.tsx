    ...
    <div className="mx-auto max-w-6xl space-y-6">
      <section className="grid overflow-hidden border border-[var(--shell-border)] bg-[var(--shell-panel)] xl:grid-cols-[1.2fr_0.8fr]">
        <div className="p-6 sm:p-8">
          <div className="flex items-start justify-between gap-5">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-400">训练方向</p>
            ...
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-white"><Target className="h-5 w-5" /></div>
          </div>
        ...
        <div className="border-t border-[var(--shell-border)] bg-[var(--shell-hover)] p-6 xl:border-l xl:border-t-0 sm:p-8">
          ...
          <div className="mt-5 space-y-5">
            {['保存目标岗位与 JD', '上传或编辑简历', '开始针对性模拟与复练'].map((item, index) => <div key={item} className="flex gap-3"><span className="text-sm font-semibold text-blue-600 dark:text-blue-400">0{index + 1}</span><p className="text-sm leading-6 text-[var(--shell-muted)]">{item}</p></div>)}
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <section className="border border-[var(--shell-border)] bg-[var(--shell-panel)] p-5 sm:p-6">
          <div className="flex items-center gap-2"><Plus className="h-4 w-4 text-blue-600 dark:text-blue-400" /><h2 className="text-lg font-semibold text-[var(--shell-text)]">新增岗位目标</h2></div>
          <p className="mt-2 text-sm leading-6 text-[var(--shell-muted)]">尽量粘贴完整 JD。职责与要求越具体，后续训练越贴近真实面试。</p>
          <form onSubmit={create} className="mt-6 space-y-4">
            <label className="block"><span className="mb-2 block text-sm font-medium text-[var(--shell-text)]">岗位名称</span><input required maxLength={120} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} placeholder="例如：Java 后端工程师" className="w-full rounded-lg border border-[var(--shell-border)] bg-[var(--shell-bg)] px-3 py-2.5 text-sm text-[var(--shell-text)] outline-none placeholder:text-[var(--shell-subtle)] focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15" /></label>
            <label className="block"><span className="mb-2 block text-sm font-medium text-[var(--shell-text)]">公司 <span className="font-normal text-[var(--shell-subtle)]">可选</span></span><input maxLength={120} value={form.company} onChange={(event) => setForm({ ...form, company: event.target.value })} placeholder="例如：某科技公司" className="w-full rounded-lg border border-[var(--shell-border)] bg-[var(--shell-bg)] px-3 py-2.5 text-sm text-[var(--shell-text)] outline-none placeholder:text-[var(--shell-subtle)] focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15" /></label>
            <label className="block"><span className="mb-2 block text-sm font-medium text-[var(--shell-text)]">职位描述</span><textarea required maxLength={30000} value={form.jobDescription} onChange={(event) => setForm({ ...form, jobDescription: event.target.value })} placeholder="粘贴职责、任职要求和加分项…" className="min-h-44 w-full resize-y rounded-lg border border-[var(--shell-border)] bg-[var(--shell-bg)] px-3 py-2.5 text-sm leading-6 text-[var(--shell-text)] outline-none placeholder:text-[var(--shell-subtle)] focus:border-blue-500 focus:ring-2 focus:ring-blue-500/15" /></label>
            {error && <p className="text-sm text-red-600 dark:text-red-300">{error}</p>}
            <button disabled={saving} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700 disabled:opacity-60">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <ChevronRight className="h-4 w-4" />}{saving ? '正在保存…' : '保存并用于训练'}</button>
          </form>
        </section>
      </div>
    </div>

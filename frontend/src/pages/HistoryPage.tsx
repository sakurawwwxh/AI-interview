...
function statusMeta(resume: ResumeListItem) {
  ...
  if (resume.analyzeStatus === 'PROCESSING') return { label: '正在分析', className: 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-300' };
...
export default function HistoryList({ onSelectResume }: HistoryListProps) {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    <header className="flex flex-wrap items-end justify-between gap-5 border-b border-[var(--shell-border)] pb-6">
      <div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-blue-600 dark:text-blue-400">训练资料</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-[var(--shell-text)]">简历不是文件，是训练的起点。</h1><p className="mt-2 text-sm text-[var(--shell-muted)]">查看分析状态、进入详情、从任意一份简历继续模拟面试。</p></div>
      <Link to="/upload" className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700"><Plus className="h-4 w-4" />上传简历</Link>
    </header>

    <section className="border border-[var(--shell-border)] bg-[var(--shell-panel)] px-5 py-4 sm:flex sm:items-center sm:justify-between sm:px-6"><div className="flex gap-7"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--shell-subtle)]">资料数量</p><p className="mt-1 text-2xl font-semibold text-[var(--shell-text)]">{resumes.length}</p></div><div className="border-l border-[var(--shell-border)] pl-7"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--shell-subtle)]">已完成分析</p><p className="mt-1 text-2xl font-semibold text-blue-600 dark:text-blue-400">{completedCount}</p></div></div><p className="mt-4 text-sm text-[var(--shell-muted)] sm:mt-0">点击任意一行查看简历、报告和面试记录。</p></section>
